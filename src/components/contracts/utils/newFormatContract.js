...
export const contractInfo = (contractData = defaultContractInfo) => {
    ...
    const motherEmail = contractData.guardians.find(g => g.guardianType === "Mother")?.email || "___________________________________";
    const fatherEmail = contractData.guardians.find(g => g.guardianType === "Father")?.email || "_________________";
    ...
    const totalCost = registrationFee + 
    ...
    Object.keys(programCounts).reduce((sum, program) => {
        return sum + (programPrices[program] * (programCounts[program] || 0));
    }, 0);
    ...
    return {
        page1: {
            ...
            parr4: `\nLunes: ${schedule.mondayStartLabel} a ${schedule.mondayEndLabel}     Martes: ${schedule.tuesdayStartLabel} a ${schedule.tuesdayEndLabel}    Miércoles: ${schedule.wednesdayStartLabel} a ${schedule.wednesdayEndLabel}`,
            parr5: `\nJueves: ${schedule.thursdayStartLabel} a ${schedule.thursdayEndLabel}    Viernes: ${schedule.fridayStartLabel} a ${schedule.fridayEndLabel}`,
