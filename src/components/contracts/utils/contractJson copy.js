...
export const contractInfo = (contractInfo = defaultContractInfo) => {
  return {
    page1: {
      ...
      parr6: `${contractInfo?.childrenInfo
        ?.map(
          child =>
            `- Nombre del niño: ${child.name}, fecha de nacimiento: ${child.bornDate}`
        )
        .join(`\n`)}`,
    ...
    page2: {
      ...
      parr8: `\n-Pago anual por actividades de $20 por familia (${
        contractInfo?.childrenInfo?.length
      } niños): Total $${contractInfo?.childrenInfo?.length * 20}`,
      parr9: `\n-Infant (bebés de 6 semanas a 18 meses): por (${
        contractInfo?.childrenInfo?.filter(child => child.ageGroup === 'Infant')
          .length
      } niños): Total $ \n -Toddler (18 meses a 3 años): por (${
        contractInfo?.childrenInfo?.filter(
          child => child.ageGroup === 'Toddler'
        ).length
      } niños): Total $ \n -Pre-school (3 a 5 años): por (${
        contractInfo?.childrenInfo?.filter(
          child => child.ageGroup === 'Pre-school'
        ).length
      } niños): Total $ \n -School age (5 a 12 años): por (${
        contractInfo?.childrenInfo?.filter(
          child => child.ageGroup === 'School age'
        ).length
      } niños): Total $`,
    ...
    page11: {
      ...
      parr1: `Yo, ${
        contractInfo?.guardian?.name
      }, Madre/Padre o Apoderado de mi niño(s) llamado(s) ${
        contractInfo?.childrenInfo?.map(child => child.name).join(', ') ??
        'No hay niños asignados'
      }, estoy de acuerdo que Educando Childcare: (Por favor marque lo que desee)`,
      parr2: `\n${
        contractInfo.photosAllowed ? '☑' : '☐'
      } Comparta fotos, videos, media con mi niño(s) allí con otras familias de niños registrados en el daycare para propósito solo familiar y personal (via email, libro del año o de su aula o impresión de fotos).`,
      parr3: `\n${
        contractInfo.externalPhotosAllowed ? '☑' : '☐'
      } Permiso a otros Padres de los niños registrados en Educando Childcare para tomar fotografías, videos y media con mis niños allí, si están de acuerdo otros Padres para uso solamente personal y familiar como Celebraciones de cumpleaños, festividades celebradas en el daycare.`,
      parr4: `\n${
        contractInfo.specialExternalUsage ? '☑' : '☐'
      } Uso de fotografías de mi niño para trabajos de artes manuales y actividades para familias de niños registrados en el daycare.`,
      parr5: `\n${
        contractInfo.externalUsageAllowed ? '☑' : '☐'
      } Uso de fotografías, video, media con mi niño allí para promover Educando Childcare Center.`,
    ...
    page12: {
      ...
      parr2: `\n${
        contractInfo.walkAroundNeighborhood ? '☑' : '☐'
      } Caminar alrededor del vecindario de Educando Childcare.`,
      parr3: `\n${
        contractInfo.walkToThePark ? '☑' : '☐'
      } Caminar hacia parque del vecindario o tal vez ir en el transporte de la guardería.`,
      parr4: `\n${
        contractInfo.walkAroundSchool ? '☑' : '☐'
      } Caminar en la escuela del vecindario.`,
    ...
    page13: {
      subtitle: {
        text: `RECIBO DEL MANUAL DE PADRES DE EDUCANDO CHILDCARE\n`,
