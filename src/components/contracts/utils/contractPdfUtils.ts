...
const addInputField = (
  ...
  doc.text(
    inputValue,
    options.marginLeft + doc.getTextWidth(content) + 5,
    options.yPosition
  );
...
const addSeparator = (doc: jsPDF, options: Options) => {
  ...
  doc.line(
    options.marginLeft,
    yPosition,
    options.pageWidth - options.marginLeft,
    yPosition
  );
...
const addSignSpaces = (
  ...
  signOptions: any = DefaultSignOptions,
  ...
  if (signOptions.showParentName) {
    nameLines.forEach((line: string, index: number) => {
      // Explicitly type 'line' as string and 'index' as number
      ...
      doc.text(
        line,
        options.marginLeft,
        options.yPosition + (index === 0 ? 12 : index * 5)
      );
    });
  }
  ...
  const signEducandoLabel =
    language === Language.Spanish
      ? 'Firma de Educando Childcare Center'
      : 'Educando Childcare Center Signature';
  if (signOptions.showParentName) {
    doc.text(
      Functions.formatDateToMMDDYY(new Date()),
      options.marginLeft + nameFieldWidth + signatureFieldWidth + 10,
      lineYPosition - 2
    );
    doc.text(
  ...
  tokens.forEach(token => {
    switch (token) {
      ...
      case '$':
        ...
        doc.setFont(
          'Helvetica',
          doc.getFont().fontStyle === 'italic' ? 'normal' : 'italic'
        );
        break;
      case '#':
...
export class ContractPdf {
  static contractBuilder = (
    contractInformation: ContractInfo = mockContract,
...
const addFirstPage = (
  ...
  options: Options,
  contractInfo: any,
  _contractInformation: ContractInfo,
  language: Language
) => {
  options.yPosition = options.yPosition - 10;
  const contractLabel: string =
    language === Language.English ? 'Contract' : 'Contrato';
...
function processChildIterations(
  page: PageContent,
  children: any[]
): PageContent[] {
  ...
  return children.map(child => {
    ...
    const processedPage: PageContent = JSON.parse(
      JSON.stringify({
      ...
      Object.entries(obj).forEach(([key, value]) => {
        if (typeof value === 'string') {
          obj[key] = value
            ...
            .replace(
              /{{childBornDate}}/g,
              Functions.formatDateToMMDDYY(child.born_date!) || ''
            )
            .replace(
              '{{healthStatus}}',
            ...
            .replace(
              '{{treatment}}',
              child.medicalInformation?.treatment ||
                '________________________________________________________'
            )
            .replace(
              '{{allergies}}',
              child.medicalInformation?.allergies ||
                '_________________________________________________'
            )
            .replace(
              '{{instructions}}',
            ...
            .replace(
              '{{feedingTimes}}',
              (
                child.formulaInformation?.feedingTimes || [
                  '________',
                  '________',
                  '________',
                  '________',
            ...
            .replace(
              '{{maxBottleTime}}',
              child.formulaInformation?.maxBottleTime || '_______________'
            )
            .replace(
              '{{minBottleTime}}',
              child.formulaInformation?.minBottleTime || '_________'
            )
            .replace(
              '{{bottleAmount}}',
              child.formulaInformation?.bottleAmount ||
                '________________________________'
            )
            .replace(
              '{{feedingInstructions}}',
              child.formulaInformation?.feedingInstructions ||
                '__________________________________________________________________________________________________________________________________________________________________________________________________'
            )
            .replace(
              '{{otherFood}}',
              child.formulaInformation?.otherFood ||
                '___________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________'
            )
            .replace(
              '{{foodAllergies}}',
              child.formulaInformation?.foodAllergies ||
                '______________________________________________________________________________________________________________________________________________________'
            )
            // Follow meal program with unicode support
            .replace(
              '{{followMealProgramYes}}',
              child.formulaInformation?.followMealProgram ? icons.check : ''
            )
            .replace(
              '{{followMealProgramNo}}',
              child.formulaInformation?.followMealProgram ? '' : icons.none
            )
            .replace(
              '{{soap}}',
              child.permissionsInformation?.soap ? icons.check : icons.none
            )
            .replace(
              '{{sanitizer}}',
              child.permissionsInformation?.sanitizer ? icons.check : icons.none
            )
            .replace(
              '{{rashCream}}',
              child.permissionsInformation?.rashCream ? icons.check : icons.none
            )
            .replace(
              '{{teethingMedicine}}',
              child.permissionsInformation?.teethingMedicine
                ? icons.check
                : icons.none
            )
            .replace(
              '{{sunscreen}}',
              child.permissionsInformation?.sunscreen ? icons.check : icons.none
            )
            .replace(
              '{{insectRepellent}}',
              child.permissionsInformation?.insectRepellent
                ? icons.check
                : icons.none
            )
            .replace(
              '{{other}}',
              child.permissionsInformation?.other ||
                '______________________________________________________________________________________________________________________________________________________'
            );
  ...
  switch (true) {
    ...
    case key.startsWith('signSectionEducando'):
      addSignSpaces(
        ...
        options,
        contractInformation,
        { showParentName: false, showEducandoName: true },
        language
      );
      break;
    case key.startsWith('signSection'):
      addSignSpaces(
        ...
        options,
        contractInformation,
        { showParentName: true, showEducandoName: false },
        language
      );
      break;
