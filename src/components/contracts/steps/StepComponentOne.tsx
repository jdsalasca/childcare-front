...
export const StepComponentOne: React.FC<StepComponentOneProps> = ({
  ...
  useEffect(() => {
    ...
    if (isLoading) {
      ...
      if (
        loadingInfo.loading ||
        loadingInfo.loadingMessage !== loadingMessage
      ) {
        setLoadingInfo({
          loading: false,
  ...
  const onHandlerChildBackendAsync = async (data: {
    ...
    try {
      ...
      const validResponses = responses.filter(
        (response): response is ChildType => response !== undefined
      );
  ...
  const onSubmit = async (data: { children: ChildType[] }) => {
    ...
    try {
      ...
      const mergedChildren = childrenAsync.map(child => ({
        ...child,
        medicalInformation:
          existingChildrenMap.get(child.id)?.medicalInformation ||
          child.medicalInformation,
        formulaInformation:
          existingChildrenMap.get(child.id)?.formulaInformation ||
          child.formulaInformation,
        permissionsInformation:
          existingChildrenMap.get(child.id)?.permissionsInformation ||
          child.permissionsInformation,
      }));
  ...
  const handleBornDateChange = (date: Date | undefined, index: number) => {
    ...
    if (currentProgram !== program) {
      customLogger.debug('Updating program', {
        currentProgram,
        newProgram: program,
      });
  ...
  const handleChildSelect = (e: { value: ChildType }) => {
    ...
    const existingChildIndex = fields.findIndex(child => {
      if (child.static_id && selectedChild.static_id) {
        ...
        return (
          (child.name + child.last_name).toLowerCase() ===
          (selectedChild.name + selectedChild.last_name).toLowerCase()
        );
      }
      return false;
    });
    ...
    if (existingChildIndex === -1) {
      ...
      setValue(
        `children.${existingChildIndex}.born_date` as const,
        new Date(selectedChild.born_date!)
      );
    }
  };
        ...
        {fields.map((child, index) => (
          <div key={child.id} className='child-form'>
            ...
            <CalendarWrapper
              ...
              showIcon
              onChangeCustom={(value: Date) =>
                handleBornDateChange(value, index)
              }
            />
            <InputTextWrapper
