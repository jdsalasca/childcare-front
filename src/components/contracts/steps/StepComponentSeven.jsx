...
const StepComponentSeven = ({
  ...
  /** important as an array buffer to be converted to pdf */
  const onGenerateContractWithGob = async (contractInformation = defaultContractInfoFinished) => {
    ...
    const externalPdfBytes = ContractPdf.contractVersion1;
    // const externalPdf = await PDFDocument.load(externalPdfBytes)
    const externalPdf = await PDFDocument.load(externalPdfBytes);
    // Copy all pages from external PDF
    const copiedPages = await jsPdfDocument.copyPages(
      externalPdf,
      externalPdf.getPageIndices()
    )
    // Add copied pages to jsPDF
    copiedPages.forEach(page => {
      jsPdfDocument.addPage(page)
    })
    customLogger.debug('BEFORE mergedPdfBytes', copiedPages)
  ...
  const handleDownloadPdf =async (language = 'en') => {
    ...
    if (
      ...
      onGenerateContractWithGob(contractInformation)

      // setReceiptBase64(ContractPdf.contractGenerator(contractInformation))
    }
  }
