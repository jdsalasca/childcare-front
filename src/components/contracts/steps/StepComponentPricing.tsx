  ...
  return children.reduce(
    (acc, child) => {
      ...
      if (ageInYears < AGE_RANGES.INFANT.max) {
        ...
        ageInYears >= AGE_RANGES.TODDLER.min &&
        ageInYears < AGE_RANGES.TODDLER.max
      ) {
        acc.toddlerCount++;
      } else if (
        ageInYears >= AGE_RANGES.PRESCHOOL.min &&
        ageInYears < AGE_RANGES.PRESCHOOL.max
      ) {
        acc.preschoolCount++;
...
const StepComponentPricing: React.FC<StepComponentPricingProps> = ({
  ...
  const hasExistingCounts =
    ...
    Object.keys(contractInformation.serviceCounts).length > 0;
  ...
  useEffect(() => {
    if (contractInformation.children && !hasExistingCounts) {
      const counts = contractInformation.children.reduce(
        (acc, child) => {
          const age = child.age || 0;
          if (age >= AGE_RANGES.INFANT.min && age < AGE_RANGES.INFANT.max)
            acc.infantCount++;
          else if (
            age >= AGE_RANGES.TODDLER.min &&
            age < AGE_RANGES.TODDLER.max
          )
            acc.toddlerCount++;
          else if (
            age >= AGE_RANGES.PRESCHOOL.min &&
            age < AGE_RANGES.PRESCHOOL.max
          )
            acc.preschoolCount++;
          else if (age >= AGE_RANGES.SCHOOL.min && age <= AGE_RANGES.SCHOOL.max)
            acc.schoolCount++;
          return acc;
        },
                ...
                {[
                  {
                    ...
                    rate: PRICES.REGISTRATION_FEE,
                  },
                  {
                    name: 'activityCount',
                    label: t('activityFee'),
                    rate: PRICES.ACTIVITY_FEE,
                  },
                ].map((fee, index) => (
                  <motion.div
                    key={fee.name}
                  ...
                  >
                    <div className='flex items-center justify-between gap-3'>
                      ...
                      <div className='text-right font-medium text-gray-900 min-w-[80px]'>
                        ...
                        {(
                          watchAllFields[
                            fee.name as keyof typeof watchAllFields
                          ] * fee.rate
                        ).toFixed(2)}
                      </div>
                    </div>
                ...
                {[
                  {
                    name: 'infantCount',
                    label: t('infantCare'),
                    rate: PRICES.RATES.INFANT,
                  },
                  {
                    name: 'toddlerCount',
                    label: t('toddlerCare'),
                    rate: PRICES.RATES.TODDLER,
                  },
                  {
                    name: 'preschoolCount',
                    label: t('preschoolCare'),
                    rate: PRICES.RATES.PRESCHOOL,
                  },
                  {
                    name: 'schoolCount',
                    label: t('schoolCare'),
                    rate: PRICES.RATES.SCHOOL,
                  },
                  {
                    name: 'transportationCount',
                    label: t('transportation'),
                    rate: PRICES.TRANSPORTATION,
                  },
                ].map((service, index) => (
                  <motion.div
                    key={service.name}
                  ...
                  >
                    <div className='flex items-center justify-between gap-3'>
                      ...
                      <div className='text-right font-medium text-gray-900 min-w-[80px]'>
                        ...
                        {(
                          watchAllFields[
                            service.name as keyof typeof watchAllFields
                          ] * service.rate
                        ).toFixed(2)}
                      </div>
                    </div>
