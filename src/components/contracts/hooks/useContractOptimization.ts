...
export const useContractOptimization = ({
  ...
  const updateGuardians = useCallback(
    (guardians: Guardian[]) => {
      setContractInformation({
        ...contractInformation,
        guardians,
  ...
  const updateContractField = useCallback(
    (field: keyof ContractInfo, value: any) => {
      setContractInformation({
        ...contractInformation,
        [field]: value,
      });
    ...
    (stepIndex: number): boolean => {
      switch (stepIndex) {
        case 0: // Children
          return (
            childrenCount > 0 &&
            Boolean(contractInformation.children?.[0]?.first_name)
          );
        case 1: // Guardians
          return (
            guardiansCount > 0 &&
            Boolean(contractInformation.guardians?.[0]?.name)
          );
        case 2: // Permissions
