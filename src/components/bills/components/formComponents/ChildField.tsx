...
const ChildFormField: FC<ChildFormFieldProps> = ({
  ...
  useEffect(() => {
    ...
    const prevCash =
      typeof prevBillRef.current.cash === 'number'
        ? prevBillRef.current.cash
        : typeof prevBillRef.current.cash === 'string' &&
            prevBillRef.current.cash !== ''
          ? parseFloat(prevBillRef.current.cash)
          : 0;

    const prevCheck =
      typeof prevBillRef.current.check === 'number'
        ? prevBillRef.current.check
        : typeof prevBillRef.current.check === 'string' &&
            prevBillRef.current.check !== ''
          ? parseFloat(prevBillRef.current.check)
          : 0;
