...
const MigrateBills: React.FC = () => {
  ...
  const onLookUpAvailableDatesToPick = async () => {
    ...
    if(dates.httpStatus !==200){
      ToastInterpreterUtils.toastBackendInterpreter(toast, dates, "error", t('errorLookingUpAvailableDates'));
    }
    const parsedDates = dates.response?.map((date: string) => DateUtilsImpl.getInstance().convertToUTC(date)); // Convert to Date objects
    setAvailableDates(parsedDates);
  }
  const normalMigration = async (data : MigrateBillsForm) => {
    const  total = await DailyCashRecordsAPI.normalMigration(Functions.formatDateToYYYYMMDD(data.initial_day!), Functions.formatDateToYYYYMMDD(data.target_day!))
    customLogger.debug("normalMigration", total)
  }
  const mergeMigration = async (data : MigrateBillsForm) => {
    const  total = await DailyCashRecordsAPI.mergeMigration(Functions.formatDateToYYYYMMDD(data.initial_day!), Functions.formatDateToYYYYMMDD(data.target_day!))
    customLogger.debug("mergeMigration", total)
    }
  const overwriteMigration = async (data : MigrateBillsForm) => {
    const  total = await DailyCashRecordsAPI.overwriteMigration(Functions.formatDateToYYYYMMDD(data.initial_day!), Functions.formatDateToYYYYMMDD(data.target_day!))
    customLogger.debug("overwriteMigration", total)}
 
  const onSubmit: SubmitHandler<MigrateBillsForm> =async (data : MigrateBillsForm) => {

    customLogger.debug("onSubmit", data);
    if((data.total_cash_target_day == null && data.total_check_target_day == null) && ( data.total_cash_initial_day != null || data.total_check_initial_day != null)){
      await normalMigration(data);
  }else if(data.total_cash_target_day != null && data.total_check_target_day != null && data.total_cash_initial_day != null || data.total_check_initial_day != null){
    ...
    ToastInterpreterUtils.toastInterpreter(toast, "success", t('reloaded'), t('billsMigratedSuccessfully'));
  ...
  const onChangeInitialDay = async (value: Date) => { // Adjust the type as necessary
    if(initialDayRef  != null && initialDayRef?.current != null && initialDayRef.current.toDateString() === value.toDateString()){
      return;
    }
    initialDayRef.current = value;
    const dataByDay = await DailyCashRecordsAPI.getAllRecordByDate(Functions.formatDateToYYYYMMDD(value));
    if(dataByDay.httpStatus === 200  && dataByDay.response != null && dataByDay.response.length > 0 && dataByDay.response[0].total_cash != null){
      ...
      ToastInterpreterUtils.toastInterpreter(toast, "success", t('notDataForInitialDayTitle'), "");
    }
  };

  const onChangeTargetDay = async(value: Date) => { // Adjust the type as necessary
    if(value ==null || (initialDayRef  !== null && initialDayRef.current !== null && initialDayRef.current.toDateString() === value.toDateString())){
      return value;
    }
    const dataByDay = await DailyCashRecordsAPI.getAllRecordByDate(Functions.formatDateToYYYYMMDD(value));
    if(dataByDay.httpStatus === 200  && dataByDay.response != null && dataByDay.response.length > 0 && dataByDay.response[0].total_cash != null){
      ...
      ToastInterpreterUtils.toastInterpreter(toast, "success", t('notDataForTargetDayTitle'), t('notDataForTargetDay'));
    }
    initialDayRef.current = value;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="c-form-migrate-bills">
      ...
      <Button type="submit" label={t('requestMigration')} className="p-button-success" disabled={!validMigration} />
    </form>
  );
};
