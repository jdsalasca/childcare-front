...
export const exportToPDF = (data: FormValues): void => {
  const bills: Bill[] = data.bills; // Assuming bills is an array of Bill
  const doc = new jsPDF();
  addFonts(doc);
  ...
  bills.forEach(bill => {
    if (
      (bill.cash == null || bill.cash === 0) &&
      (bill.check == null || bill.check === 0)
    ) {
      return;
    }
...
const drawGrayLine = (doc: jsPDF, yPos: number, pageWidth: number): void => {
...
const renderLeftContent = (
  ...
  const leftContent = [
    {
      text: 'DEPOSIT TICKET',
      style: { bold: true, color: 'black', drawColor: 'gray' },
    },
    {
      text: 'EDUCANDO CHILDCARE CENTER INC',
      style: { bold: true, color: 'black' },
    }, // Set drawColor to gray for this line
    ...
    {
      text: 'SIGN HERE IF CASH RECEIVED (IF REQUIRED)*',
      style: { color: 'black' },
    },
  ...
  const drawText = (
    text: string,
    style: { bold?: boolean; color?: string; drawColor?: string },
    yPos: number,
...
const renderRightContent = (
  ...
  if (bill?.cash && Number(bill?.cash) > 0) {
    doc.text((Number(bill?.cash) || 0).toFixed(2), cashBoxX + 5, yPos, {
      align: 'left',
    });
  }
  yPos += lineHeight * 0.6;
  ...
  if (bill?.check && Number(bill?.check) > 0) {
    doc.text((Number(bill?.check) || 0).toFixed(2), checkBoxX + 5, yPos, {
      align: 'left',
    });
  }
  yPos += lineHeight * 1.2;
  ...
  doc.setLineWidth(0.5); // Restore line width to 0.5
  doc.setFillColor(whiteColor[0], whiteColor[1], whiteColor[2]); // Reset to white
  doc.setFontSize(8);
  doc.setFont(undefined!, 'normal');
  ...
  doc.text(
    ((Number(bill?.cash) || 0) + (Number(bill?.check) || 0)).toFixed(2),
    totalBoxX + 5,
    yPos,
    { align: 'left' }
  );
