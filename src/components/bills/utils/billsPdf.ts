...
export const exportToPDF = (data : FormValues): void => {
  ...
  bills.forEach((bill) => {
    if((bill.cash == null || bill.cash ===0) && (bill.check == null ||  bill.check ===0) ){
      return;
    }
    ...
    renderLeftContent(doc, leftStartX, startY, lineHeightLeftSide, bill, pageWidth);
...
const renderLeftContent = (doc: jsPDF, startX: number, startY: number, lineHeight: number, bill: Bill, pageWidth: number): void => {
  const leftContent = [
  ...
  const drawText = (text: string, style: { bold?: boolean; color?: string; drawColor?: string }, yPos: number, drawColor: string = 'black', xPosition: number = 0): void => {
    if (style.bold) {
...
const renderRightContent = (doc: jsPDF, startX: number, startY: number, lineHeight: number, bill: Bill): void => {
