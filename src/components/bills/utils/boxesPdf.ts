...
export const exportBoxesToPDF = (data: FormValues): void => {
  ...
  // Add Title and Date
  doc.setFontSize(14);
  doc.text('EDUCANDO CHILDCARE CASH REGISTER', pageWidth / 2, margin, {
    align: 'center',
  });
  doc.setFontSize(12);
  ...
  currentY = generateChildrenTable(
    doc,
    data.bills!,
    currentY,
    pageHeight,
...
const generateReceiptForChild = (
  ...
  // Payment received by
  doc.text(
    `Payment received by: ____________________________`,
    margin,
    currentY
  );
  currentY += 10;
  ...
  doc.text(
    `Services provided for the weeks of: ____________ to ____________`,
    margin,
    currentY
  );
  currentY += 10;
  ...
  doc.text(
    `1 week                               $ _________ Still due        $ ___________ Credit.`,
    margin,
    currentY
  );
  currentY += 10;
...
const generateCashOnHandSection = (
  ...
  if (data.closedMoneyData && data.closedMoneyData.has_closed_money) {
    values.push({
      label: 'Closed Money',
      value: `$${Number(
        data.closedMoneyData.total_closing_amount
...
const generateCashCountTable = (
  ...
  if (
    data.closedMoneyData?.closing_details &&
    Array.isArray(data.closedMoneyData.closing_details)
  ) {
    ...
    data.closedMoneyData?.opening_details &&
    Array.isArray(data.closedMoneyData.opening_details)
  ) {
