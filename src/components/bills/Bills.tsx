  ...
  ({
    ...
    const cash =
      typeof bill.cash === 'number'
        ? bill.cash
        : typeof bill.cash === 'string' && bill.cash !== ''
          ? parseFloat(bill.cash)
          : 0;
    const check =
      typeof bill.check === 'number'
        ? bill.check
        : typeof bill.check === 'string' && bill.check !== ''
          ? parseFloat(bill.check)
          : 0;
    ...
    const getFormErrorMessage = useCallback(
      (fieldName: string) => {
        ...
        return (
          fieldError && <small className='p-error'>{fieldError.message}</small>
        );
      },
      [errors.bills, index]
    );
        ...
        {summaryItems.map((item, index) => (
          ...
          >
            <div className='flex items-center gap-3 mb-3'>
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${item.color.replace('text-', 'bg-').replace('-600', '-100')}`}
              >
      ...
      <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6'>
        ...
        <div className='mb-6'>
          ...
          <p className='text-gray-600 text-lg'>
            {t(
              'bills.manageBillsDescription',
              'Manage daily payments and generate reports'
            )}{' '}
            • {filteredBills.length} {t('bills.total', 'total')}
          </p>
        </div>
    ...
    () => (
      <div className='text-center py-8'>
        ...
        <p className='text-gray-500 mb-4'>
          {blockContent
            ? t(
                'bills.selectDateFirst',
                'Please select a date to start adding bills'
              )
