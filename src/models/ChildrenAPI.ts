...
const ChildrenAPI = {
  ...
  ): Promise<ApiResponse<ChildType>> => {
    try {
      const response = await API.post<ChildType>(
        BASE_URL,
        '/children',
        childData
      );
  ...
  // Get all children
  getChildren: async (): Promise<ApiResponse<ChildType[]>> => {
    // Change the response type to Child[]
    try {
  ...
  updateChild: async (
    ...
    try {
      const response = await API.put<ChildType>(
        BASE_URL,
        `/children/${id}`,
        childData
      );
