...
interface IBillTypeAPI {
  getBillTypesByCurrencyCode: (
    currencyCode: string
  ) => Promise<ApiResponse<BillType[]>>;
}
...
const billTypeAPI: IBillTypeAPI = {
  ...
  getBillTypesByCurrencyCode: async (
    ...
    try {
      const response = await API.get<BillType[]>(
        BASE_URL,
        '/bill_types/by_currency',
...
export const useBillTypesByCurrencyCode = (
  currencyCode: string
): UseQueryResult<BillType[], Error> => {
  return useQuery<BillType[], Error>({
    queryKey: ['billTypes', currencyCode],
    queryFn: async (): Promise<BillType[]> => {
      const billTypesResponse =
        await billTypeAPI.getBillTypesByCurrencyCode(currencyCode);
