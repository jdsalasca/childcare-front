...
export const ContractPermissionsAPI = {
  createContractPermissions: async (
  ...
  updateContractPermissions: async (
    contractPermission: ContractPermission
  ): Promise<any> => {
    try {
      const response = await API.put(BASE_URL, '/contract_permissions', {
        contractPermission,
      });
      return response;
    } catch (error) {
...
export class ContractPermissionsValidator {
   ...
   */
  static getValidContractPermissions(
    permissions: ContractPermission | undefined
  ): number {
    if (permissions == null) {
      return 0;
    }
    const permissionValidKeys = Object.keys(permissions).filter(key => {
      ...
      return (
        permissions[typedKey] === true &&
        ContractModel.CONTRACT_PERMISSIONS.includes(key as string)
      );
    });
